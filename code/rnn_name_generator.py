...
class RNNLM(nn.Module):
    ...
    def forward(self, x, h_last=None):
        ...
        if (h_last != None):
            ...
            h = torch.zeros((1, batch_size, self.gru_size), dtype=torch.float)
        # x = x.long()

        embeds = self.emb(x)  # B * S * emb_size
        out, h = self.gru(embeds, h)
        # out: B * S * gru_size
        # h: num_layers(1 by default) * B * gru_size
...
def train_model(model, Xtrain, Ytrain, Xval, Yval, id_to_char, max_epoch):
    ...
    for e in range(max_epoch):
        ...
        for n in range(num_batches):
            ...
            yh, hidden = model(Xtrain[s:t], hidden)
            hidden = hidden.data
            loss = lossfn(yh.permute(0, 2, 1), Ytrain[s:t])
            optim.zero_grad()
...
def gen_string(model, id_to_char, max_len=20, sample=True):
    ...
    for i in range(max_len):
        ...
        if sample:
            ...
            prob = np.array(f(out[0][0]).detach().data)
            indexs = np.arange(len(prob))

            char_ind = np.random.choice(indexs, p=prob)

            ch = id_to_char[int(char_ind)]
            x[0][0] = char_ind
        else:
            inp = np.array(out.detach().data[0][0])
            char_ind = np.argmax(inp)
            ch = id_to_char[int(char_ind)]
            x[0][0] = char_ind
        if ch == '.':
            break
